...
{
	std::strcpy(inputFileName, input);
	std::strcpy(outputFileName, output);
	sec = scanInterval;
}

void FlightsScanner::handleAirport(char* airport) const
{
	if (std::strlen(airport) != 3)
...
void FlightsScanner::parseLine(const char* line, FlightRecord& record) const
{
	...
	{
		std::cout << "Error while processing the destination. Given: " << record.dest << std::endl;//none
		throw;
	}
	...
	try
	{
		convertToNumber(amountStr);
	}

	catch (const std::bad_cast& e)
	{
		static const unsigned DEFAULT_VALUE = 50;

		std:: cout<< "Invalid amount. Setting to default value: " << DEFAULT_VALUE << std::endl;//weak
		record.amount = DEFAULT_VALUE;
...
bool FlightsScanner::readAndExportData(std::ifstream& input)
{
	...
	if (!ofs.is_open())
	{
		std::cout << "Could not open export file!" << std::endl;
		return false;
	}
